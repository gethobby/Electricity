...
function loadNodeInfo(nodeid)
{
	if(nodeid){
		$("#NodeID").val(nodeid);
		$.ajax({
	    	url: '/Electricity/GetDetailInfo?infotype=node&id='+nodeid+"&tid="+Math.random(),
...
function loadResourceInfo(nodeid)
{
	if(nodeid){
		$("#NodeID").val(nodeid);
		$.ajax({
	    	url: '/Electricity/GetDetailInfo?infotype=resource&id='+nodeid+"&tid="+Math.random(),
	    	success:function(data){	
	    		if(data.length>0){
...
function modifyResourceinfo()
{
	...
	var updateurl="/Electricity/UpdateNodeinfo?type=resource&id="+$("#NodeID").val();
	if(accountID){
...
function modifyNodeinfo()
{
	...
	var jsonObject=new Object();
	var updateurl="/Electricity/UpdateNodeinfo?type=node&id="+nodeID;
