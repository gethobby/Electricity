...
function showModelFiles(source)
{
	...
	if(modelfiles.style.display=="none"){//如果模型文件部分为隐藏状态
		...
		$.ajax({
		     type: "POST",   
		     url: "/Electricity/GetModellist?objId="+TargetObjectID,   
...
function jianmo(){
	...
	if(ModelfileID){
		$.ajax({
			type: "post",
			url:'/Electricity/GetAvailableSoft?fileID='+ModelfileID+"&nodeIP="+nodeIP,//模型文件ID（文件名fileinfo_ID,文件所属nodeIP）
			success:function(data){
...
function insertNewObjectRecord()
{
	...
	if(checkInput()){
		...
		$.ajax({   
		     type: "POST",   
		     url: "/Electricity/CreateNewRecord?type=object",   
		     data:jsonObject,
		     //traditional: true,
...
function downloadModelFile(modelName) {
	...
	if(modelName!=null){
		...
		$.ajax({   
		     type: "POST",   
		     url: "/Electricity/DownloadModelFile?modelName="+modelName+"&nodeIP="+nodeIP,   
		     //traditional: true,
		     success:function(str_response){
		    	 if("success"==str_response)
		    	 {
		    		...
		    		// 指定磁盘的模型文件(从存储节点机下载的压缩模型文件)复制到webcontent目录
		    			$.ajax({   
		    			     type: "POST",   
		    			     url: "/Electricity/UploadServlet?type=copy&modelName="+modelName, 
		    			     //traditional: true,
		    			     success:function(str_response){
		    			    	 if("success"==str_response)
		    			    	 {
...
function upmodelfile()
{
	...
	if (nodeIP!=null) {
	   ...
	   jsonObject.modelName=document.getElementById("ObjectName").value;
		...
		$.ajax({
		     type: "POST",   
		     url: "/Electricity/CreateNewRecord?type=modelfile",   
		     data:jsonObject,
		     success:function(str_response){
