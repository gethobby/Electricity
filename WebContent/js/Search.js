...
function showFilteredFiles(source)
{
	...
	if(modelfiles.style.display=="none"){//如果模型文件部分为隐藏状态
		...
		$.ajax({
		     type: "POST",   
		     url: "/Electricity/AdvancedSearch?searchtype=modelfile",   
		     data:jsonObject,
...
function loadSearchObject()
{
	...
	$.ajax({
	     type: "POST",   
	     url: "/Electricity/AdvancedSearch?searchtype=object&tid="+Math.random(),   
	     data:jsonObject,
	     success:function(data){
...
function loadSearchSoft()
{
	...
	$.ajax({
	     type: "POST",   
	     url: "/Electricity/AdvancedSearch?searchtype=soft&tid="+Math.random(),   
	     data:jsonObject,
	     success:function(data){
	    	 if(data&&data.length>0){
